...
description: Diagnosis loop for unclear, hard, flaky, or performance-sensitive bugs. Use when the user asks to diagnose/debug, when a repro or feedback loop is needed, or when a reported failure's cause/fix is not already obvious.
---
...
Read [quality-baseline.md](../../references/quality-baseline.md) before claiming completion.
...
- [ ] Throwaway prototypes cleaned up, or a cleanup plan is proposed when deletion/movement would be a separate side effect
- [ ] The hypothesis that turned out correct is stated in the handoff, commit message, or PR description — so the next debugger learns
